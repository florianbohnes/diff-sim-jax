...
class TrackLine:
    def __init__(self, map_name, config_map, racing_line=False, expand=False) -> None:
    ...
    def load_centerline(self):
        ...
        filename = self.config_map.map_path + f'{self.map_name}/{self.map_name}_centerline_vel_newconv.csv'
        out = np.loadtxt(filename, delimiter=self.config_map.wpt_delim, skiprows=self.config_map.wpt_rowskip)
        # s_m	 x_m	 y_m	 psi_rad	 kappa_radpm	 vx_mps
        xs = out[:, self.config_map.wpt_xind]
        ys = out[:, self.config_map.wpt_yind]

        # add the first point to the end to make a loop
        xs = np.append(xs, [0.0])
        ys = np.append(ys, [0.0])
    ...
    def plot_vehicle(self, point, theta):
        ...
        plt.arrow(point[0], point[1], np.cos(theta), np.sin(theta), color='g', head_width=0.1, head_length=0.1,
                  linewidth=2)
