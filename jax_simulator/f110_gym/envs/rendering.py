...
class EnvRenderer(pyglet.window.Window):
    ...
    def __init__(self, width, height, *args, **kwargs):
        ...
        # glClearColor(196 / 255, 196/255,  196 / 255, 1.)
        ...
        self.score_label = pyglet.text.Label(
            'Lap Time: {laptime:.2f}, Ego Lap Count: {count:.0f}, Ego Vel: {vel:.0f}'.format(laptime=0.0, count=0.0, vel=0.0),
            font_size=36,
            x=0,
            y=0,
            # anchor_x='left',
    ...
    def update_obs(self, obs):
        ...
        # self.score_label.text = f'Lap Time: {obs["lap_times"][0].round(2)}, Ego Lap Count: {obs["lap_counts"][self.ego_idx]}'
        self.score_label.text  = 'Lap Time: {laptime:.2f}, Ego Lap Count: {count:.0f}, Ego Vel: {vel:.1f}'.format(laptime=obs["lap_times"][0], count=obs["lap_counts"][self.ego_idx], vel=obs["linear_vels_x"][self.ego_idx])
